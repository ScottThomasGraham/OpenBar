# Evora emulator workbench

Headless EdgeTX **simulator** for the TX16S, used to develop and screenshot the Evora UI
...
`edgetx-home.png` / `edgetx-menu.png` — the current **baseline** (stock EdgeTX `2.12.1-evora`),
captured live from this harness. As Evora's bespoke UI lands, these get replaced by Evora screens.

## How it works

A dedicated arm64 Linux container (`evora-simu`) builds the EdgeTX `simu` SDL target **natively**
(no qemu → no compiler segfaults), runs it under **Xvfb** (virtual display), and grabs the framebuffer
with ImageMagick `import`. The Evora-TX repo is bind-mounted at `/src`.

### One-time container setup
```bash
docker --context colima-evora run -d --name evora-simu --platform linux/arm64 \
  -v "$HOME/Projects/Evora/forks/evora-tx:/src" -w /src arm64v8/ubuntu:24.04 sleep infinity
docker exec evora-simu bash -lc 'apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y \
...
docker exec evora-simu bash -lc 'python3 -m pip install --break-system-packages \
  asciitree jinja2 pillow lz4 pyelftools pydantic clang'
```

### Build the simulator
```bash
docker exec evora-simu bash -lc 'cd /src && mkdir -p build-simu && cd build-simu && \
...
```bash
docker cp scripts/shot.sh evora-simu:/root/shot.sh
docker exec evora-simu timeout 45 bash /root/shot.sh /root/sim/out.png <navkeys...>
docker cp evora-simu:/root/sim/out.png ./out.png
```
