...
_Design law for Evora. 2026-06-01. Do not drift from this._

## The Xbox-controller model
The Evora radio is a **universal controller**, like an Xbox controller. You never change stick
directions, channel maps, reverses, trims, mixes, or endpoints on an Xbox controller — the **game**
decides what the inputs do. For Evora, the "game" is the **flybarless unit running Rotorflight**.

Consequences:
- **The radio's channel output is FIXED and IDENTICAL for every Evora user.** One standard channel
...
## The Evora standard channel map (decided 2026-06-01)

Evora's radio outputs this **fixed 12-channel layout for every user** — the de-facto Rotorflight
...
### What Evora bakes into the standard Rotorflight preset (FlyDragon)
So a freshly-flashed Evora heli is flyable with **zero channel config**:
- `set rc_map = AECR1T23`
...
This requires a **standardized Evora config baked into Rotorflight for the FlyDragon** (a Rotorflight
preset/target shipped/flashed with Evora). Key engineering task on the FBL/link side. The user never
sees or sets channel order — it just works.

## Binding flow (the Evora pairing experience)
...
3. The user taps the **Link / chain button** on the radio → Evora scans and lists **all receivers
   currently in bind mode**.
4. The user taps the receiver → Evora auto-sets the binding (phrase or equivalent) → **paired.**
